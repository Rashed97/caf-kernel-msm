...
static ssize_t fwu_sysfs_custom_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
struct synaptics_rmi4_fwu_handle {
	...
	struct f34_flash_ctrl_status flash_ctrl_status;
...
static struct device_attribute attrs[] = {
	...
	__ATTR(dispconfigblockcount, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(custom_config_id, S_IRUGO,
			fwu_sysfs_custom_config_id_show,
			synaptics_rmi4_store_error),
};
...
static int fwu_read_f34_flash_ctrl_status(void)
{
	int retval;
	unsigned char data;

	int flash_ctrl_status_offset = fwu->f34_fd.data_base_addr + BLOCK_DATA_OFFSET+ fwu->block_size;

	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			flash_ctrl_status_offset,
			&data,
			sizeof(data));
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"%s: Failed to read flash reg\n",
				__func__);
		return retval;
	}

	/* Program enabled bit not available - force bit to be set */
	fwu->flash_ctrl_status.program_enabled = 1;
	fwu->flash_ctrl_status.status = (data >> 4) & MASK_3BIT;
	fwu->flash_ctrl_status.command = data & MASK_4BIT;
...
static int fwu_write_f34_command(unsigned char cmd)
{
	...
	fwu->flash_ctrl_status.command = cmd;
...
static int fwu_wait_for_idle(int timeout_ms)
{
	...
	do {
		...
		if (count == timeout_count)
			fwu_read_f34_flash_ctrl_status();

		if ((fwu->flash_ctrl_status.command == 0x00) &&
				(fwu->flash_ctrl_status.status == 0x00))
			return 0;
...
static int fwu_enter_flash_prog(void)
{
	...
	if (!fwu->flash_ctrl_status.program_enabled) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		snprintf(firmware_name, 50, "synaptics_%s.img", fwu->rmi4_data->chip_name);
		dev_dbg(&fwu->rmi4_data->i2c_client->dev,
...
static ssize_t fwu_sysfs_custom_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	memcpy(buf, fwu->custom_config_id, sizeof(fwu->custom_config_id));
	return sizeof(fwu->custom_config_id);
}
